const validarvalor = {
	plaza: /^[a-zA-ZÀ-ÿ\s]{1,40}$/, // Letras y espacios, pueden llevar acentos.
...
const validarFormulario = (e) => {
	switch (e.target.name) {
		...
		break;
        case 'plaza':
            validar(validarvalor.plaza, e.target, e.target.name);
		break;
        case 'correo':
		...
		break;
        case 'titulo':
            validar(validarvalor.titulo, e.target, e.target.name);
		break;
        case 'nit':
            validar(validarvalor.nit, e.target, e.target.name);
		break;
        case 'salario':
            validar(validarvalor.salario, e.target, e.target.name);
		break;
        case 'fecha':
            validar(validarvalor.fecha, e.target, e.target.name);
		break;
	}
}
